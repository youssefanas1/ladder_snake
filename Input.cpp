...
CellPosition Input::GetCellClicked() const
{
	...
	if (UI.InterfaceMode == MODE_DESIGN)
	{
		if (y >= UI.ToolBarHeight && y <= (UI.height - UI.StatusBarHeight))
		{
			if (x > 0) {
				int W = UI.width / 11;
				int H = (y - UI.ToolBarHeight) / UI.CellHeight;
				cellPos.SetVCell( H);
				cellPos.SetHCell(x / W);
			}
			else {
				cellPos.SetHCell(-1);
				cellPos.SetVCell(-1);
			...
			///TODO: SetHCell and SetVCell of the object cellPost appropriately//       using the coordinates x, y and the appropriate variables of the UI_Info Object (UI)
		}
	}
